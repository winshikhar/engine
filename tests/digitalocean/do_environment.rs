...
fn deploy_a_working_environment_with_no_router_on_do() {
    engine_run_test(|| {
        ...
        let context = context();
        let _context_for_delete = context.clone_not_same_execution_id();
        ...
        let _ea_delete = EnvironmentAction::Environment(environment_for_delete);
        ...
        "deploy_a_working_environment_with_no_router_on_do".to_string()
    })
}
...
fn deploy_a_working_environment_router_and_app_on_do() {
    engine_run_test(|| {
        ...
        "deploy_a_working_environment_router_and_app_on_do".to_string()
    })
}
